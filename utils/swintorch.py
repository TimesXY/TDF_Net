...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, input_resolution, num_heads, qkv_bias=True,
        ...
        # Define a linear layer that transforms the input vector into a key and value.
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # kv(): -> [batch_size * num_windows, Wh * Ww, 2 * total_embed_dim]
        # reshape: -> [batch_size * num_windows, Wh * Ww, 2, num_heads, embed_dim_per_head]
        # permute: -> [2, batch_size * num_windows, num_heads, Wh * Ww, embed_dim_per_head]
        ...
        # learned parameter q with dimensions [num_windows, num_heads, Wh * Ww, embed_dim_per_head]
        batch_size = int(
            b_ / (self.input_resolution[0] * self.input_resolution[1] / self.window_size[0] / self.window_size[0]))
        q = nn.Parameter(torch.randn(1, k.size(1), k.size(2), k.size(3))).to("cuda")

        # Use the torch.repeat_interleave function to repeat q batch_size times in the 0-th dimension
        q = q.repeat_interleave(k.size(0), dim=0)
...
class WSAM(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4., qkv_bias=True,
        ...
        self.attn = WindowAttention(dim, window_size=to_2tuple(self.window_size),
                                    input_resolution=self.input_resolution, num_heads=num_heads,
...
if __name__ == '__main__':
    ...
    windows_msm = WSAM(dim=96, input_resolution=(14, 14), num_heads=4, window_size=7, shift_size=0,
...
"""
   动机: 为什么用 Windows Self-Attention Module ?   
   回复: WSAM的主要思想是将输入的图像分割成不重叠的窗口, 然后在每个窗口内部进行自注意力计算, 从而关注每个窗口内的细节特征, 如乳腺结节的边缘细节和纹
        理等, 这种局部信息聚合的方法能够有效提取图像的局部特征. 同时, 卷积网络固有的局部性和平移不变性是一种强先验信息, 因此卷积网络的归纳偏置通常
        具有更好的可转移性和泛化能力. 基于此, 为了进一步增强该模块提取局部特征的能力, 所提模块借鉴了卷积网络的学习方法, 引入了可学习的查询机制将多头
        自注意力机制调整为类似卷积的自适应聚合操作, 使得模块能够在保持注意力机制表达能力的同时利用卷积的局部性和平移不变性. 
        
        F: (Q × K^T) / sqrt(dk)
        Q: 可学习的查询变量 K: 局部特征, 关键信息 V: 信息本身. 其中, query 和 key 的点积可以解释为 key 在 query 方向上的标量投影. 因此, 
        token value 根据其与 query 的相对方向进行聚合. 更直观的说, 现在可以提取 key, 使得特征相关的 key 将与 q 紧密对齐, 这意味着网络可以优
        化 query 方向来检测局部上下文相关的特征. 这个过程在概念上类似于卷积核，因为学习的查询决定如何聚合局部特征信息, 实现对窗口内细节特征的重点关
        注. # 同时, 这种可学习的查询在不同窗口内是不共享的, 这种线性空间复杂性能够有效保持注意力机制的特征提取能力. #
"""
