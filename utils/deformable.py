...
"""
   动机: 为什么用 Deformable Self-Attention Module ?   
   回复: 近年来, 基于Transformer的网络结构在医学图像诊断任务中表现出了卓越的性能. 与卷积神经网络相比, 自注意力机制的全局感受野使得Transformer具
        备了较强的全局表征能力. 然而, 由于组织运动和自身结构等因素的影响, 乳腺结节在超声成像中具有复杂的几何变化. 传统的自注意力机制只能在固定位置和
        方向上提供注意力权重, 且易于受到非感兴趣区域特征的影响. 因此, 传统自注意力机制无法很好的从超声图像中捕获到几何变化多样的乳腺结节表征. 为了解
        决该问题, 本文采用了可变形自注意力模块 (DSAM) 在提取图像全局特征信息的同时使网络的注意力能够适应乳腺结节的不规则变形. 可变形自注意力模块的
        核心思想通过学习特定的偏移量在全局范围下调整采样点的位置, 从而实现对特定区域的加权, 使得注意力机制可以在不同的位置和方向上提供注意力. 因此, 
        可变形自注意力模块能够将网络的注意力集中在信息丰富的显著位置(乳腺结节区域), 更好的适应超声图像中乳腺结节的非刚性几何变换(复杂的乳腺结节形状和尺寸). 

        q = x * w_q; k^ = x * w_k; v^ = x * w_v; ▲p = θ(q); x^ = Φ(x; p + ▲p)
        首先, 在特征图上均匀的生成参考位置点, 然后利用线性映射将输入特征投影为全局的 query token. 随后, 将 query token 输入到偏移生成网络 θ,
        获得参考位置点的偏移量, 从而根据偏移量将候选的 key 和 value 移动到特征图中的重要位置. 接下来, 利用线性映射将变形点的采样特征投影为变形的 
        key 和 value, 将采样点映射为相对位置偏置. 最后, 将获得的query、key、value和相对位置偏置输入到多头自注意力网络进行计算, 得到最终输出z. 
        
        然而, 不同的 query token 在全局自注意力模块中通常会导致几乎完全相似的注意力模式. 因此, 在可变形自注意力模块中, 当前像素无需与所有特征像
        素进行交互, 而只需要和采样点位置的采样特征进行交互. 这种全局稀疏的注意力机制能够在有效降低计算复杂度的同时使模型快速关注到全局范围下的重要特
        征区域, 从而获取到更细致的全局特征信息.
        
        因此, 可变形自注意力模块通过偏移量对全局范围下的注意力权重进行变形, 进一步增强了注意力机制的全局表征能力, 使得网络能够更好的捕捉超声图像中乳
        腺结节的形状和尺寸等全局特征信息, 实现对超声图像中的重要区域的关注.
        
        然而, 可变形自注意力模块虽然可以以数据依赖的方式动态的选择自注意力中键值对的位置, 但其选择过程是在全局范围内进行的. 也就是说, 网络会在全局范
        围内寻找最相关的区域, 然后将注意力集中在这些相关区域上, 从而更好的捕获全局特征信息. 因此, 该模块的本质仍然是关注图像的全局信息 [DAT].
        
        可视化结果: 目标区域的offset会呈现出一定的集中趋势.
"""
