...
"""
   动机: 为什么用可逆神经网络 Invertible Neural Network, 
   回复: 在传统神经网络的前向传播过程中, 网络的隐藏层会对输入数据进行各种形式的非线性变换, 但这些不可逆的非线性变换不可避免的会导致传输信息的丢失. 因
   此, Dinh等人在神经网络设计中引入了可逆性, 建立了可逆神经网络. 可逆神经网络的结构决定了其输入可以从输出中恢复, 因此, 可逆神经网络可以在不丢失任何
   信息的情况下进行前向传播, 实现输入特征和输出特征之间的无损可逆信息映射. 因此, 我们采用可逆神经网络实现不同模态特征之间的相互转换, 网络的可逆性完美
   的契合了不同模态特征之间相互转换的需求. 
   
   在训练过程中, 通过损失函数计算真实模态特征和转换模态特征之间的KL散度, 从而促使可逆神经网络模块学习不同模态特征之间的映射关系, 实现各模态特征的相互
   转换. 在推理过程中, 当模态缺失时, 利用训练好的可逆神经网络模块对缺失模态的特征进行恢复, 从而完成模态缺失状态下的诊断过程.
         
   可逆性分析:
     雅可比行列式不为0. 若 y = f(x) 可逆, 则 x = f-1(y), 且其雅可比矩阵相乘为单位矩阵. 
                     因此, 其数据分布可表示为: p(y) = g(x)|det(J_(f-1))| = g(x)(1/|det(J_f)|, 因此, 网络的雅可比行列式不为0.
     什么是雅可比式(Jacobian): 雅可比行列式是以n个n元函数的偏导数为元素的行列式. 在函数都连续可微的前提下(即偏导数都连续), 它就是函数组微分形式下
     的系数矩阵的行列式。
   
   补充:
      是否需要利用可逆残差网络的可逆损失实现不同模态之间的相互转换. -- Revtorch
      可逆神经网络模块能够很好地恢复缺失的模态, 使得恢复结果和原始输入之间的概率分布非常接近. 使网络能够在不增加参数的情况下, 实现多模态的融合和恢复
